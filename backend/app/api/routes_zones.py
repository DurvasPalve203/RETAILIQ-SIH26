...
def get_all_zones():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT zone_id, camera_id, polygon_json, zone_type, label, target_sku_id, baseline_image_path, expected_capacity, created_at FROM zones")
...
def calibrate_zones(zones: List[ZoneCreate]):
    ...
    for z in zones:
        ...
        """, (z.zone_id, z.camera_id, poly_json, z.zone_type, z.label, z.target_sku_id, z.expected_capacity, now))
    conn.commit()
