...
class PipelineCoordinator:
    ...
    def _pipeline_loop(self):
        while self._running:
            ...
            # 6. Module A & B: Queue Formation Detection & Wait-Time Prediction
            ...
            for zid, qstate in self.latest_queue_states.items():
                ...
                if q_pred["estimated_wait_seconds"] >= 240.0 or qstate["queue_length"] >= 3:
                    ...
                    # Clear/resolve queue alerts if queue shortened
            ...
            annotated_raw = self._render_live_annotations(
                frame, zones, det_results, tracking_results, zone_statuses, queue_results, queue_predictions, norm_metrics
            )
            self.latest_raw_frame = annotated_raw

            # 8. Module C: Apply Privacy-Preserving Face Blur
            ...
            self.latest_privacy_split_frame = self.face_blur.generate_split_screen_demo(annotated_raw, blurred_frame)

            # 9. Compile System State
            now = time.time()
            self._frame_count += 1
    ...
    def _render_live_annotations(
        ...
        for z in zones:
            ...
            if ztype == "queue_zone":
                color = (255, 180, 20) # Cyan-orange for queue zone
        ...
        for tr in tracking_results["tracks"]:
            ...
            if q_info and q_info["classification"] == "in_queue":
                ...
                tag_col = (0, 220, 255) # Cyan
            elif q_info and q_info["classification"] == "browsing":
                tag = "Browsing"
                tag_col = (100, 220, 100) # Green
            else:
                tag = f"ID #{tid}"
                tag_col = (255, 200, 0) # Gold
        ...
        fps_str = f"RetailIQ Edge | FPS: {self.inference_fps} | Queue Wait: {wait_str} | Footfall: {tracking_results['total_footfall_today']}"
        cv2.putText(vis, fps_str, (12, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 240, 180), 1)
        ...
        if norm_metrics["is_occluded"]:
            cv2.putText(vis, "[OCCLUSION WARNING]", (w - 420, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (50, 50, 255), 2)
